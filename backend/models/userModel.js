...
const userSchema = new mongoose.Schema({
  name: { 
    ...
    trim: true // Names ke extra space automatic remove karega
  },
  email: { 
    type: String, 
    required: [true, "Email address is required"], 
    unique: true, 
    lowercase: true, // "AYUSH@email.com" -> "ayush@email.com" conversion
    trim: true,
    match: [/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/, "Please provide a valid email address"]
  },
  image: {
    type: String,
    // Base64 text hata kar universal cloud CDN asset path inject kiya hai
   default:
  ...
  gender: { 
    ...
    enum: ["Male", "Female", "Other", "Not Selected"] // Data inputs restrictions strict checks
  },
  dob: { 
    type: String, 
    default: "Not Selected",
  ...
  minimize: false,
  timestamps: true // Automatic 'createdAt' aur 'updatedAt' track karega analytics pipelines ke liye
});

// Database queries filter scanning speeds badhane ke liye indexing metrics placement
